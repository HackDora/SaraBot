angular.module('elastichat', ['ionic', 'monospaced.elastic', 'angularMoment'])
    ...
    .controller('UserMessagesCtrl', ['$scope', '$rootScope', '$state',
        '$stateParams', 'BotService', '$ionicActionSheet',
        '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval',
        function($scope, $rootScope, $state, $stateParams, BotService,
                 $ionicActionSheet,
                 $ionicPopup, $ionicScrollDelegate, $timeout, $interval) {
            ...
            $scope.sendMessage = function(sendMessageForm) {
                ...
                //BotService.sendMessage(message).then(function(data) {
                $scope.input.message = '';
    ...
    // services
    .factory('BotService', ['$http', '$q',
        function($http, $q) {
            var bot = {};

            bot.sendSessionId = function(sessionId){
                var endpoint =
                    'https://api.motion.ai/messageBot?msg=emptystring&bot=16889&session='+sessionId+'&key=4c3591b222ea196b863523cffea230f2';
            ...
            bot.sendMessage = function(sessionId, input){
                var endpoint =
                    'https://api.motion.ai/messageBot?msg=' + input + '&bot=16889&session='+sessionId+'&key=4c3591b222ea196b863523cffea230f2';
                console.log(endpoint);
                return $http.get(endpoint).then(function(response) {
                    return response.data;
                }, function(err) {
